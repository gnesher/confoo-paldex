...
/** Reactive composable -- returns a ref that is true when the given Pal is on the team. */
export function useIsInTeam(palId: Ref<string> | (() => string)) {
  return useStore(teamStore, (state) => {
    const id = typeof palId === 'function' ? palId() : palId.value
    return state.pals.some((p) => p.id === id)
  })
}
